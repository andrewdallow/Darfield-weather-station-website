...
$query = "SELECT UNIX_TIMESTAMP(LogDate) AS LogDate, LowHum, HighHum FROM Dayfile WHERE LogDate >= DATE_FORMAT(CURDATE() - INTERVAL " . $interval . " MONTH, '%Y-%m-%d') ORDER BY LogDate ASC";
...
while ($row = $result->fetch_row()) {
    $date[] = $row[0];
    $data[] = array((float) $row[1], (float) $row[2]);
}
