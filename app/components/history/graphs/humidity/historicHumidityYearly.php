...
while($rowYear = $resultYears->fetch_row()) {
    ...
    $query = "SELECT ROUND(AVG(LowHum)), ROUND(AVG(HighHum)), MONTH(LogDate) FROM Dayfile WHERE LogDate >= '" . $currYear . "-01-01' AND LogDate < '" . $currYear1 . "-01-01' GROUP BY DATE_FORMAT(LogDate,'%c') ORDER BY DATE_FORMAT(LogDate,'%m')";
    ...
    while($row = $result->fetch_row()) {
        ...
        while ((int)$m < (int)$row[2]) {
            $valTot[$m - 1] = array(0, 0);
            $m++;
        }
        $valTot[] = array((float)$row[0], (float)$row[1]);
        $m++;
    }
    $ret['datasets'][] = [
            "name"=>"$currYear",
