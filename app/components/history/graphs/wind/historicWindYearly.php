...
while($rowYear = $resultYears->fetch_row()) {
    ...
    $query = "SELECT ROUND(AVG(HighAvgWSpeed),1), MONTH(LogDate) FROM Dayfile WHERE LogDate >= '" . $currYear . "-01-01' AND LogDate < '" . $currYear1 . "-01-01' GROUP BY DATE_FORMAT(LogDate,'%c') ORDER BY DATE_FORMAT(LogDate,'%m')";
